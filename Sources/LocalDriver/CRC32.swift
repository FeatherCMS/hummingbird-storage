...
private extension FixedWidthInteger where Self: UnsignedInteger {

    func bytes(usingBigEndian: Bool = true) -> [UInt8] {
        var _endian = usingBigEndian ? bigEndian : littleEndian
...
final class CRC32: ChecksumCalculator {
    ...
    func update(_ bits: [UInt8]) {
        for bit in bits {
            let idx = Int((hash ^ UInt32(bit)) & 0xFF)
            hash = (hash >> 8) ^ lookupTable[idx]
        }
    }
   
    func finalize() -> String {
        let hash = hash ^ 0xFFFF_FFFF
        return Data(hash.bytes()).base64EncodedString()
    }
}
